                    ...
                    object : OnCompleteListener<AuthResult> {
                        override fun onComplete(p0: Task<AuthResult>) {
                            if (p0.isSuccessful) {
                                ...
                                var kaydedilecekKullanıcı =
                                    Users(gelenEmail,"",password,nameSurname,userName,"",userID)
                ...
                auth.createUserWithEmailAndPassword(fakePhone,password).addOnCompleteListener(object :
                    ...
                    override fun onComplete(p0: Task<AuthResult>) {
                        if (p0.isSuccessful) {
                            ...
                            var userID = auth.currentUser!!.uid.toString()
                            var kaydedilecekKullanıcı = Users("", phoneNumber, password, nameSurname, userName, fakePhone, userID)
                            ref.child("users").child(userID).setValue(kaydedilecekKullanıcı)
                                .addOnCompleteListener(object : OnCompleteListener<Void> {
                                    override fun onComplete(p0: Task<Void>) {
                                        if (p0.isSuccessful) {
